...
with open(
    tfold.config.data_dir + "/msas/mhc_msa_index.pckl", "rb"
) as f:  # index of precomputed MSAs for MHCs
    mhc_msa_index = pickle.load(f)
with open(
    tfold.config.data_dir + "/msas/pmhc_msa_index.pckl", "rb"
) as f:  # index of pMHC assay MSAs
...
def _prefilter_registers(logkds, threshold):
    return logkds["tail"][
        logkds["logkd"] - np.min(logkds["logkd"]) < np.log10(threshold)
    ]
...
def _make_af_inputs_for_one_entry(x, use_mhc_msa, use_paired_msa, tile_registers):
    ...
    if use_mhc_msa:
        try:
            msas_filenames = _get_mhc_msa_filenames(
                x["class"], "A", mhc_A_query.data["pdbnum"]
            )
    ...
    if x["class"] == "II":
        ...
        if use_mhc_msa:
            try:
                msas_filenames = _get_mhc_msa_filenames(
                    x["class"], "B", mhc_B_query.data["pdbnum"]
                )
    ...
    for tails in x["templates"].columns:
        ...
        z = x["templates"][tails].map(
            lambda y: template_tools.make_template_hit(
                x["class"], y, pep_query, mhc_A_query, mhc_B_query
            )
        )
    ...
    if (
        tile_registers
    ):  # order as #[tail1_templ1,tail2_templ1,...], split into consecutive groups of <=templates_per_run
        z = templates_processed.values
        z = z.reshape(z.shape[0] * z.shape[1])
        lz = len(z)
        templates_by_run = [
            z[templates_per_run * i : templates_per_run * (i + 1)]
        ...
        for c in templates_processed.columns:
            ...
            templates_by_run += [
                z[templates_per_run * i : templates_per_run * (i + 1)]
                for i in range(
                    lz // templates_per_run + int(lz % templates_per_run != 0)
                )
            ]
    ...
    for run in templates_by_run:
        ...
        # paired msa
        msas_pmhc = []
        if use_paired_msa and (
            len(tails) == 1
        ):  # only use when there is one register in the run
            try:
                pmhc_msa_parts = [
                    _get_pmhc_msa_filenames(x["class"], "P", pdbnum),
                    _get_pmhc_msa_filenames(
                        x["class"], "M", mhc_A_query.data["pdbnum"]
                    ),
                ]
                if x["class"] == "II":
                    pmhc_msa_parts.append(
                        _get_pmhc_msa_filenames(
                            x["class"], "N", mhc_B_query.data["pdbnum"]
                        )
                    )
                msas_pmhc += [
                    {i: f for i, f in enumerate(pmhc_msa_parts)}
                ]  # {0:pep,1:M,2:N}
            except Exception as e:
                print(x["class"], "P", pdbnum,x["class"], "M", mhc_A_query.data["pdbnum"])
        ...
        input_data["target_id"] = x[
            "pmhc_id"
        ]  # pmhc id of query: add from index if not present!
...
def map_one_mhc_allele(a):
    ...
    # HLA class 1
        ...
        try:
            ...
            print(f"Cannot split MHC allele {a}. Please check format.")
            mhc_b = None
            mhc_a = None
        if mhc_a is None:
            print(f"Cannot find MHC {a}. Please check format.")
...
def map_one_mhc_seq(s):
    if "/" in s:
        ...
        try:
            ...
            print(f"{s} aligned, mhc_a is: {mhc_a}")
        except Exception:
...
def prepare_mhc_objects(df):
    if "MHC sequence" in df:
        ...
        raise ValueError(
            'Cannot find columns "MHC allele" or "MHC sequence" in input data.'
        )
    df[["mhc_a", "mhc_b", "class"]] = df[k].map(f).tolist()
    # only if class == I, remove rows with pep length is shorter than 8
    df = df[
        df.apply(lambda x: len(x["pep"]) >= 8 if x["class"] == "I" else True, axis=1)
    ]
    return df
...
def preprocess_chunk(chunk, mhc_as_obj, use_seqnnf):
    if "pmhc_id" not in chunk.columns:  # assign ids
        chunk["pmhc_id"] = chunk.index.copy()

    if not mhc_as_obj:
        chunk = prepare_mhc_objects(chunk)
    else:
        print("MHC objects from columns mhc_a and mhc_b will be used.")
    return chunk


def preprocess_df(df, mhc_as_obj=False, use_seqnnf=False, num_cores=None):
    """
    takes a df with columns "pep", "MHC allele" or "MHC sequence";
    adds pmhc_id if not present;
    adds MHC NUMSEQ objects in columns mhc_a and mhc_b (skip if mhc_as_obj=True);
    ...
    if num_cores is None:
        num_cores = 1  # mp.cpu_count()  # Use all available cores by default
    ...
    with mp.Pool(num_cores) as pool:
        # Apply the preprocess_chunk function in parallel
        results = pool.map(
            partial(preprocess_chunk, mhc_as_obj=mhc_as_obj, use_seqnnf=use_seqnnf),
            df_split,
        )

    # Combine the processed chunks back into a single DataFrame
    df = pd.concat(results)
    # drop all rows with None in mhc_a or pep
    df = df.dropna(subset=["mhc_a", "pep"])
    print(df.shape)
...
def make_inputs(df, params={}, date_cutoff=None, print_stats=False):
    ...
    df["tails_prefiltered"] = df.apply(
        lambda x: _prefilter_registers(
            x["seqnn_logkds_all"], params[x["class"]]["kd_threshold"]
        ),
        axis=1,
    )
    # assign templates
    df["templates"] = df.apply(
        lambda x: template_tools.assign_templates(
            x["class"],
            x["pep"],
            pep_tails=x["tails_prefiltered"],
            mhc_A=x["mhc_a"].data,
            mhc_B=_mhcb(x),
    ...
    inputs_dicts = df.apply(
        lambda x: _make_af_inputs_for_one_entry(
            x,
            params[x["class"]]["use_mhc_msa"],
            params[x["class"]]["use_paired_msa"],
            params[x["class"]]["tile_registers"],
        ),
        axis=1,
    )
    inputs = []
    ...
    if print_stats:
        for cl in ["I", "II"]:
            if reg_counts[cl]:
                ...
                print(f"class {cl}:")
                print(
                    "pmhcs: {:3d}; runs: {:4d}, runs per pmhc: av {:3.1f}, max {:2d}; registers per pmhc: av {:3.1f}, max {:2d}".format(
                        queries,
                        runs,
                        runs / queries,
                        max(run_counts[cl]),
