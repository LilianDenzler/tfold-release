...
# pep renumbering
cl_I_resnum_template_left = ["   0{:1d}".format(x) for x in range(1, 10)] + [
    "{:4d} ".format(x) for x in range(1, 6)
]
...
cl_II_resnum_template = ["   0{:1d}".format(x) for x in range(1, 10)] + [
    "{:4d} ".format(x) for x in range(1, 10000)
]
...
def _make_pep_pdbnums_I(pep_len, left_tail, right_tail):
    ...
    assert (
        -1 <= left_tail <= 9
    ), "cl I pep: left tail length should be between -1 and 9;"
    assert (
        0 <= right_tail <= 9999
    ), "cl I pep: right tail length must be between 0 and 9999;"
    core_len = pep_len - left_tail - right_tail  # core length
    assert core_len >= 8, "cl I pep: core length must be at least 8;"
    assert (
        core_len <= 18
    ), "cl I pep: core too long;"  # cannot index cores longer than 18
    left_part = cl_I_resnum_template_left[
        9 - left_tail :
    ]  # e.g. [9:] for no tail, [10:] for tail=-1 (i.e. from res 2)
    l_insert = max(0, core_len - 9)
    l_insert_right = l_insert // 2
    l_insert_left = l_insert - l_insert_right
    center_part = (
        cl_I_resnum_template_insert[:l_insert_left]
        + cl_I_resnum_template_insert[9 - l_insert_right :]
    )
    right_part = cl_I_resnum_template_right[
        max(0, 9 - core_len) :
    ]  # remove res 6 if core length 8
...
def _make_pep_pdbnums_II(pep_len, left_tail):
    ...
    pdbnum = (
        cl_II_resnum_template_ext[len(cl_II_resnum_template_ext) - left_tail_ext :]
        + cl_II_resnum_template[max(0, 9 - left_tail) :]
    )
    return pdbnum[:pep_len]
...
def _pdbnum_to_tails(pdbnum):
    ...
    return np.sum(pdbnum < "   2 ") - 1, np.sum(
        pdbnum > "   9 "
    )  # res 2 and 9 always present
...
def renumber_pep(pdb):
    ...
    p_pdbnum = list(
        structure_dict["P"].keys()
    )  # add sort? no, in case very long pep with [a-z] indexed left tail
    pep_len = len(p_pdbnum)
    pep_coords = np.array(
        [_get_CA_coord_w_default(structure_dict["P"][k]) for k in p_pdbnum]
    )
...
def compute_rmsds(pdb, pdb_id):
    ...
    pepseq = "".join(
        [
            pdb_tools.aa_dict.get(x.get_resname(), "X")
            for x in structure["P"].get_residues()
        ]
    )
    true_pdb_path = f"{true_pdb_dir}/{pdb_id}.pdb"
    structure_ref, _ = pdb_tools.parse_pdb(true_pdb_path, "true")
    pepseq_ref = "".join(
        [
            pdb_tools.aa_dict.get(x.get_resname(), "X")
            for x in structure_ref["P"].get_residues()
        ]
    )
    ...
    if "N" in structure_ref_dict:
        ...
        cl = "I"
    pdb_tools.superimpose_by_chainmap(
        structure, structure_ref, chainmaps[cl], CA_only=True, verbose=False
    )
    # align peptide sequences, make resmap
    y = pairwise2.align.globalms(
        pepseq, pepseq_ref, match=1, mismatch=-1, open=-1, extend=-1
    )[0]
    i1, i2 = 0, 0
    resmap = []
    ...
    if cl == "II":  # restrict to ext core
        resmap = [
            ...
            if (a[1] >= "P   09")
            and (a[1] <= "P   9 ")
            and not re.search("[a-z]", a[1])
        ]
    pep_rmsd = pdb_tools.rmsd_by_resmap(
        structure, structure_ref, resmap, allow_missing_res=True, verbose=False
    )
    mhc_rmsd = pdb_tools.rmsd_by_chainmap(
        structure, structure_ref, chainmaps[cl], verbose=False
    )
    return {
        "pep_CA": pep_rmsd["CA"],
        "pep_all": pep_rmsd["all"],
        "mhc_CA": mhc_rmsd["CA"],
        "mhc_all": mhc_rmsd["all"],
    }
